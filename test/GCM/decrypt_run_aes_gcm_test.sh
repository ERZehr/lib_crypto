...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  if [[ "$line" =~ ^COUNT ]]; then
    ...
    echo "Running: $AES_BIN GCM $TMP_INPUT $KEY $IV $ENC_DEC -ctr $COUNTER_HEX -aad $AAD -tag $TAG"
    ./main GCM "$TMP_INPUT" "$KEY" "$IV" "$ENC_DEC" "-ctr" "$COUNTER_HEX" "-aad" "$AAD" "-tag" "$TAG" > "$TMP_RESULT"
